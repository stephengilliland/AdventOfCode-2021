inp = [["fgcae", "ebafc", "cabdef", "eg", "abecfg", "abgfed", "feg", "gafdc", "bceg", "ebgcadf", ":", "defagbc", "faecg", "cfdag", "gecb"],
...
for x in range(len(inp)):
    ...
    for y in range(len(seven)):
        if seven[y] not in one:
            a = seven[y]
            print("a =", a)
            foundLetters.add(a)
            break

    # find 6 by finding a only missing piece from 1
    inCtr = 0
    for y in range(10):
        if len(inp[x][y]) == 6:
    ...
    for y in range(10):
        if len(inp[x][y]) == 5:
            ...
            inCtr = 0
        """
    # Find C, whatever is in 1 but not six
    for y in range(2):
        if one[y] not in six:
            c = one[y]
            print("c =", c)
    ...
    for y in range(10):
        if len(inp[x][y]) == 5:
            if c not in inp[x][y]:
                print("5",inp[x][y])
                for z in range(len(six)):
                    if six[z] not in inp[x][y] and six[z] != c:
                        five = inp[x][y]
                        e = six[z]
                        foundLetters.add(e)
                        print("e =", e)
    ...
    # figure out nine
    for y in range(10):
        if len(inp[x][y]) == 6 and e not in inp[x][y]:
            nine = inp[x][y]
            break
    
    # figure out g
    for y in range(len(nine)):
        if nine[y] != a and nine[y] not in four:
            g = nine[y]
            print("g =", g)
            foundLetters.add(g)
            break
    ...
    for y in range(10):
        if len(inp[x][y]) == 5:
            if e not in inp[x][y]:
                for z in range(5):
                    print(inp[x][y][z])
                    if(inp[x][y][z] != c and inp[x][y][z] in five):
                        inCtr += 1
                if inCtr == 4:
                    three = inp[x][y]
                    break
    ...
    inCtr = 0
    # find d
    print("found letters before d", foundLetters)
    for y in range(len(three)):
        for z in range(len(inp[x][y])):#z
            if inp[x][y][z] not in foundLetters and inp[x][y][z] in three:
                foundLetters.add(inp[x][y][z])
                d = inp[x][y][z]
                print("d =", d)
                break

    # find b
    print("found before 4", foundLetters)
    for y in range(4):
        if four[y] not in foundLetters:
            foundLetters.add(four[y])
            b = four[y]
    ...
    for y in range(10):
        if len(inp[x][y]) == 6 and d not in inp[x][y]:
            zero = inp[x][y]
    ...
    for y in range(11, 15):
        ...
        inp[x][y] = sorted(inp[x][y])
        inp[x][y] = "".join(inp[x][y])
        ...
        if inp[x][y] == zero:
            ...
            print("Not found",inp[x][y])
            print("0",zero,"1",one,"2",two,"3",three,"4",four,"5",five,"6",six,"7",seven,"8",eight,"9",nine)
    newNumStr = ""
    for y in range(4):
        newNumStr += str(newNum[y])
    newNumber = int(newNumStr)
    print(newNumber)
    answer += newNumber
    numAnswers.append(newNum)
    newNum = []
print(answer)
